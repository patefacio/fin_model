//! Support for updating and signalling data.
//! Leptos provides many options for tracking updates and signaling those to
//! interested subscribers. With that approach the user moves their data into
//! either a [StoredValue](leptos::StoredValue) or some form of signal
//! like [ReadSignal](leptos::ReadSignal) or [RwSignal](leptos::RwSignal). That data
//! is housed in leptos associative arrays with _handles_ generated by leptos and
//! those handles are passed back. The handles are light-weight and _copyable_,
//! so the user can get back to the data and easily pass those handles into closures.
//!
//! The class provided here is simpler and more restrictive
//!
//! - The [Updatable] owns the data
//! - The user passes in the handler to accept the new updates
//!
//! Because it is not _Copy_ and not _Clone_ an instance of [Updatable] usually is
//! created in a parent component, moved to a child component responsible for _editing
//! the data_, and the parent gets the updates as they happen. In terms of understanding
//! the impact, the simplification is only one function is run on the update and those
//! updates do not impact the leptos reactive signaling.
//!
//! For basic updates where the parent is delegating the modification of some data to
//! a child component, prefer [Updatable]. For any updating that requires reactivity
//! prefer the leptos options - but try to keep them localized to a small number of
//! components.
//!
