...
///
/// Enumerates the types of stats tracking for generated returns in a simulation.
/// Tracking statistics of returns can help give confidence that the modeled normal
/// distributions are generating the appropriate returns. Similarly, for a simulation
/// using a _Correlated Return Generator_ stats on the resulting correlations can
/// give confidence that the process of correlating the scaled returns is working.
#[derive(Serialize, Deserialize, EnumVariantNames, EnumIter)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum ReturnStatsType {
    ///
    /// Tracks no statistics
    NoTracking = 0,
    ///
    /// Tracks statistics on returns for each dossier item as well as statistics on
    /// all modeled cross correlations for simulations using a _Correlated Return Generator_.
    /// The tracking happens first within a forecast to get incremental stats on each
    /// item's returns and incremental stats on the correlations (IncrementalPearson).
    /// Then the means of those end of forecast stats and correlations have statistics
    /// on them tracked. The result for each item is the _mean over all forecasts of the
    /// mean return of the individual forecasts_
    CorrelationsAndReturn = 1,
    ///
    /// Tracks statistics on returns for each dossier item. (See `correlations_and_returns`)
    ReturnOnly = 2,
}
impl ReturnStatsType {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            ReturnStatsType::NoTracking => "NO_TRACKING",
            ReturnStatsType::CorrelationsAndReturn => "CORRELATIONS_AND_RETURN",
            ReturnStatsType::ReturnOnly => "RETURN_ONLY",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            ...
            "CORRELATIONS_AND_RETURN" => Some(Self::CorrelationsAndReturn),
...
///
/// Enumerate start and end year of a cash flow
#[derive(Serialize, Deserialize, EnumVariantNames, EnumIter)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum YearEndpoint {
    ///
    /// Indicates change to start year
    StartYear = 0,
    ///
    /// Indicates change to end year
    EndYear = 1,
}
impl YearEndpoint {
    /// String value of the enum field names used in the ProtoBuf definition.
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            YearEndpoint::StartYear => "START_YEAR",
            YearEndpoint::EndYear => "END_YEAR",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "START_YEAR" => Some(Self::StartYear),
            "END_YEAR" => Some(Self::EndYear),
            _ => None,
        }
    }
}
